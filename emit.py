...
ARTIFACT = Template('''"""$task_name classifier. Generated by Foundry.
...
def main():
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            f"artifact subprocess exited {completed.returncode}\n{completed.stderr}"
        )

    emitted_labels = completed.stdout.strip().split("\n")
    print(f"subprocess: {sys.executable} with PYTHONHASHSEED={FRESH_HASH_SEED}")
    print(f"torch predictions: {len(torch_labels)}   artifact predictions: {len(emitted_labels)}")
    if len(emitted_labels) != len(torch_labels):
        raise ValueError("artifact returned a different number of predictions")
    ...
    for i, expected, got, text in mismatches:
        print(f"MISMATCH {i}: torch={expected} artifact={got} text={text[:80]!r}")
