...
| --- | --- | --- |
| Artifact accuracy, held out | 0.9500 | 1.0000 |
| Artifact macro F1 | 0.9514 | 1.0000 |
| Haiku 4.5 zero shot, same set | 0.9700 | 0.9800 |
| Artifact latency, median | 2.16 ms | 2.15 ms |
| Haiku latency, median serial | 842 ms | 783 ms |
| Haiku cost per 1M | 351.95 USD | 329.32 USD |
| Artifact size | 2738 KB | 2736 KB |
| Training examples | 4000 | 3980 |

Haiku accuracy moves between 0.97 and 0.98 across repeat runs on the same set.
The artifact is deterministic and returns the same answer every time.

### Against its teacher

Pooled across both tasks, 200 held out examples: the artifact gets 195 correct,
...
# verify the emitted artifact matches the trained model
...
| `emit.py` | artifact emission and the parity check |
...
hashing is randomised per process, so an artifact built with `hash` would work
in the session that created it and silently produce different features in a
fresh interpreter. The emitted artifact is verified in a subprocess with
...
| Evaluation set | n | Provenance | Artifact accuracy |
| --- | --- | --- | --- |
...
**The artifact cannot reason.** It is a single feed forward pass. It has no
